...
class OMG_PP_Helper
{
    ...
    function __construct()
    {
        ...
        $this->settings = get_option('omega_add_paypal_tracking', array());
    }
    ...
    public function delete_option($option)
    {
        ...
        if (isset($this->settings[$option])) {
            ...
            update_option('omega_add_paypal_tracking', $this->settings);
        }
    }
    ...
    public function update_options($new_options = array())
    {
        ...
        $settings = get_option('omega_add_paypal_tracking', array());

        $updated_options = array_merge($settings, $new_options);

        return update_option('omega_add_paypal_tracking', $updated_options);
    }
    ...
    public function get_config()
    {
        ...
        return array(
            ...
            'adminEmail' => get_bloginfo('admin_email'), // Public url of store homepage
            'previewUrl' => get_bloginfo('url') . '/wp-json/wc/v2/products/', // Url of API for product previews
            'rhStateApiBaseUrl' => get_bloginfo('url') . '/wp-json/omega-add-paypal-tracking/v1',
            'appId' => "1", // Name of the store
            ...
            'pluginVersion' => 'woo-commerce_' . OMG_PP_VERSION,
            ...
            'clientToken' => $this->get_option('clientToken'), // Client token for authentication in store API (eg. https://my-woo-store.com/wp-json/omegahuntereasy/state)
            ...
            'rhEasyIFrameUrl' => OMG_PP_FRONTEND_URL
        );
    }
    ...
    public function check_woocommerce_rest_api()
    {
        ...
        if ($found == 0 || empty($our_woocommerce_cust_secret)) {
            ...
            require_once(OMG_PP_DIR . 'includes/class-rh-easy-auth.php');
            $auth = new OMG_PP_Auth();
            $domain = $this->get_site_domain();
            $result = $auth->generate_keys(__('Omega Paypal Tracking', 'omega-add-paypal-tracking'), $domain, 'read_write');
    ...
    /**
     * Delete all "omega Hunter Easy - API" keys
     *
     * @return void
     * @since 0.0.5
     */
    ...
    {
        global $wpdb;
        $wpdb->query($wpdb->prepare('DELETE FROM `' . $wpdb->prefix . 'woocommerce_api_keys` WHERE `description` LIKE "%s"', '%omega paypal tracking - API%'));
    }
