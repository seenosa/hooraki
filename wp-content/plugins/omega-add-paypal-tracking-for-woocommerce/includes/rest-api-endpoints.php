...
add_action('rest_api_init', 'omega_add_paypal_tracking_register_wp_api_endpoints');
function omega_add_paypal_tracking_register_wp_api_endpoints()
{
    $controller = new OMG_PP_Custom_REST_API();
...
class OMG_PP_Custom_REST_API extends WP_REST_Controller
{
    ...
    public function register_routes()
    {
        ...
        $version = '1';
        $namespace = 'omega-add-paypal-tracking/v' . $version;
        $base = 'route';

        // http://store.com/wp-json/omega-add-paypal-tracking/v1/check
        ...
        // http://store.com/wp-json/omega-add-paypal-tracking/v1/debug
        ...
        // http://store.com/wp-json/omega-add-paypal-tracking/v1/config
        ...
        // http://store.com/wp-json/omega-add-paypal-tracking/v1/state
        ...
        // http://store.com/wp-json/omega-add-paypal-tracking/v1/google-tracking
        ...
        // http://store.com/wp-json/omega-add-paypal-tracking/v1/fb-tracking
    ...
    public function get_state($request)
    {
        ...
        if (!$this->check_clientToken($params['clientToken'])) {
            return new WP_Error('404', __('Provided clientToken is not valid or empty', 'omega-add-paypal-tracking'));
        }
    ...
    public function set_state($request)
    {
        ...
        if (count(array_intersect_key($params, $collected)) > 0) {
            ...
            return new WP_Error('404', __('Passed parameters are not supported.', 'omega-add-paypal-tracking'));
        }
    }
    ...
    /**
     * Return array of allowed omega Hunter Easy settings fields and its type
     *
     * @return array
     */
